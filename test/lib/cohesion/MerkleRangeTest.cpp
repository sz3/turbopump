...
TEST_CASE( "MerkleRangeTest/testMaxBits", "[unit]" )
{
	...
	{
		merkle_location<unsigned long long> loc(1455069594437219180ULL, 63);
		MerkleRange range(loc);
		assertEquals( loc.key, range.first() );
		assertEquals( loc.key, range.last() );
	}
...
TEST_CASE( "MerkleRangeTest/testDefault", "[unit]" )
{
...

	{
		...
		MerkleRange range(loc);
		assertEquals( 0, range.first() );
		assertEquals( 0x700000000000000ULL, range.last() );
	}


	{
		merkle_location<unsigned long long> loc(128, 0);
		MerkleRange range(loc);
	...
	{
		merkle_location<unsigned long long> loc(2112, 1);
		MerkleRange range(loc);
	...
	{
		merkle_location<unsigned long long> loc(~0ULL, 4);
		MerkleRange range(loc);
	...
	{
		merkle_location<unsigned long long> loc(0x84315, 55);
		MerkleRange range(loc);
